...
export const ProductQuickView: React.FC<ProductQuickViewProps> = ({
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-950/80 backdrop-blur-md overflow-y-auto animate-in fade-in duration-200">
      <div className={`relative w-full max-w-3xl border rounded-3xl shadow-2xl overflow-hidden my-8 ${
        ...
        <button
          type="button"
          onClick={onClose}
          className={`absolute top-4 right-4 z-20 p-2 rounded-full transition border ${
            isDark ? 'bg-slate-950/80 hover:bg-slate-800 text-slate-400 hover:text-white border-slate-800' : 'bg-slate-100 hover:bg-slate-200 text-slate-600 hover:text-slate-900 border-slate-200'
          }`}
        >
          <X className="w-5 h-5" />
        </button>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6 p-6 sm:p-8">
          ...
          <div className="space-y-4">
            ...
            <div className={`p-3 rounded-2xl border flex items-center justify-between text-xs ${
              ...
              <div className="flex items-center gap-2">
                ...
                <span>Sold by: <strong>{product.vendorName || 'Fluka Official'}</strong></span>
              </div>
          ...
          <div className="flex flex-col justify-between space-y-4">
            <div>
              ...
              <h2 className={`text-xl font-bold tracking-tight ${isDark ? 'text-white' : 'text-slate-900'}`}>{product.name}</h2>
              <p className={`text-xs mt-1 leading-relaxed ${isDark ? 'text-slate-400' : 'text-slate-600'}`}>{product.description}</p>
              ...
              <div className={`space-y-1.5 p-3 rounded-xl border text-xs ${
                isDark ? 'bg-slate-950/80 border-slate-800' : 'bg-slate-50 border-slate-200'
              }`}>
                <span className={`font-bold text-[10px] uppercase tracking-wider block mb-1 ${isDark ? 'text-slate-400' : 'text-slate-500'}`}>
                  Technical Specifications
                </span>
                {Object.entries(product.specs).map(([key, val]) => (
                  <div key={key} className="flex justify-between">
                    <span className={isDark ? 'text-slate-500' : 'text-slate-400'}>{key}:</span>
                    <span className={`font-medium ${isDark ? 'text-slate-200' : 'text-slate-800'}`}>{val}</span>
                  </div>
                ))}
              </div>
            </div>
                ...
                <button
                  ...
                  className={`p-3 rounded-xl border transition ${
                    isWishlisted
                      ...
                      : isDark ? 'bg-slate-950 border-slate-800 text-slate-400 hover:text-white' : 'bg-slate-100 border-slate-300 text-slate-600 hover:text-slate-900'
                  }`}
                >
